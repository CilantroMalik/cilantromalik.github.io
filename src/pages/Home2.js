...
export const Home2 = () => {
    ...
    const p = (str, link) => { return <span style={{color: "#ff99dd"}} onClick={() => navigate(link)} onMouseEnter={() => setHoverPage("[" + link.substring(1) + "]")} onMouseLeave={() => setHoverPage("")}><strong>{str}</strong></span> }
    ...
    return (<>
        ...
        <div style={{alignItems: "center", display: "flex", flexDirection: "column", opacity: scrollPos > 90 ? 0 : 1, transition: "opacity 0.5s ease-out"}}>
            <img style={{width: "15vh"}} src={arrow} alt="" />
            <br />
            <p><em>[scroll down, or]</em></p>
            <button style={{zIndex: "27"}} onClick={scrollDown}>enter</button>
        </div>
        ...
        <div style={{position: "fixed", top: "8vh", left: "8vw"}}>
            <h2 style={{opacity: hoverPage === "" ? 0 : 1, transition: "opacity 0.5s ease-in-out"}}><strong>{hoverPage}</strong></h2>
        </div>
        <div style={{position: "fixed", top: "18vh", right: "2vh"}}>
            <p style={{opacity: showingInst ? 1 : 0, transition: "opacity 0.5s ease-out"}}><em>(use the flowers to traverse the tree.)</em></p>
        </div>
    </>)
}